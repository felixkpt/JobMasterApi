...
";
    ...
    public async Task<string> GetCompanyReputationAsync(
        string companyName,
        string? companyWebsite = null
    )
    {
        var prompt = $"""
            You are a company reputation analyst. Provide a brief reputation summary for the company named "{companyName}"{(
                string.IsNullOrWhiteSpace(companyWebsite) ? "" : $" with website {companyWebsite}"
            )}.
            Include public perception, work culture, and industry reputation.

            Respond in a short paragraph.
            {(!_IsProd ? "Keep response under " + _TruncateCharsTo + " characters." : "")}
            """;
    ...
    public async Task<int> CalculateMatchScoreAsync(string resumeText, string jobDescription)
    {
        ...
        return (await AnalyzeJobFitAsync(Guid.Empty.ToString(), Guid.Empty, jobDescription)).Score;
    }
